...
disc <- read.delim("Disc.Regact.Boxplot.txt", header = TRUE, stringsAsFactors = FALSE)
valid <- read.delim("Valid.Regact.Boxplot.txt", header = TRUE, stringsAsFactors = FALSE)
...
# Identify the columns that contain gene RegulonActivity data.
# (Here, it is assumed that the first two columns are not gene data.)
Regact_cols <- colnames(disc)[3:ncol(disc)]
...
pdf(paste0("Regact_Boxplots_", Sys.Date(), ".pdf"), width = 18, height = 6)

# ---------------------------
# 7. Loop Through Each Gene to Create Plots
# ---------------------------
for(gene in Regact_cols) {
  ...
  disc_combined <- data.frame(RegulonActivity = disc[[gene]], ER_status = disc$`ER.status`, Dataset = "Discovery")
  valid_combined <- data.frame(RegulonActivity = valid[[gene]], ER_status = valid$`ER.status`, Dataset = "Validation")
  combined_data <- rbind(disc_combined, valid_combined)
  
  # Perform t-test on the combined METABRIC data.
  t_test_combined <- t.test(combined_data$RegulonActivity ~ combined_data$ER_status)
  ...
  # For METABRIC:
  range_combined <- diff(range(combined_data$RegulonActivity, na.rm = TRUE))
  star_pos_combined <- max(combined_data$RegulonActivity, na.rm = TRUE) + 0.05 * range_combined
  ...
  p_disc <- ggplot(disc, aes(x = `ER.status`, y = .data[[gene]], fill = `ER.status`)) +
    ...
    labs(title = paste("Discovery_" ,gene, "regulon activity")) +
  ...
  p_valid <- ggplot(valid, aes(x = `ER.status`, y = .data[[gene]], fill = `ER.status`)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.7) +
    #geom_jitter(width = 0.2, shape = 21, color = "black") +  # Add individual data points
    labs(title = paste("Validation_",gene, "regulon activity")) +
    scale_fill_manual(values = custom_colors, limits = c("Positive", "Negative")) +
    coord_cartesian(clip = "off") +
  ...
  p_combined <- ggplot(combined_data, aes(x = ER_status, y = RegulonActivity, fill = ER_status)) +
    geom_boxplot(outlier.shape = NA, alpha = 0.7) +
    #geom_jitter(width = 0.2, shape = 21, color = "black") +  # Add individual data points
    labs(title = paste(gene, "regulon activity")) +
    scale_fill_manual(values = custom_colors, limits = c("Positive", "Negative")) +
    coord_cartesian(clip = "off") +
...
write.table(results_df, file = paste0("Regact_ttest_results_", Sys.Date(), ".txt"),
            sep = "\t", row.names = FALSE, quote = FALSE)

# Save session information and all objects for future reference.
se <- sessionInfo()
save(list = ls(), file = paste0("Regact_Exp_Boxplot_", Sys.Date(), ".RData"))